---
  title: "Example: All managed forests in Saskatchewan"
---
  
```{r setup-example, include = FALSE}
```
# Example: Running spadesCBM for all managed forests of Saskatchewan {-}
Here we provide an example script to run all managed forests in Saskatchewan. This example is fairly lightweight and most computers should be able to run the simulation. 
...
## Viewing results {-}
Once the simulation has run, we can view certain results. Some of these are saved as files in the outputs folder created in the project's directory. If kept as the default in the global script above, these can be found here: `~/GitHub/spadesCBM/outputs`. You should be able to see `.rds` files for cbmPools and NPP for each simulated year, as well as various growth curve and increment related figures from [CBM_vol2biomass](https://github.com/PredictiveEcology/CBM_vol2biomass). 
...
```
# Shows yearly forest products and emissions for each simulation year
carbonPlot <- CBMutils::carbonOutPlot(
  emissionsProducts = simMngedSK$emissionsProducts
)

# Shows proportions of carbon in above and below ground compartments
