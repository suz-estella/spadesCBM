...
Throughout this manual we use an example simulation of forest carbon dynamics for the managed forests of Saskatchewan (SK) from 1985-2012 similarly to the simulations in @boisvenue2016, where the *SK* indicates the study area.
...
Note that [CBM_vol2biomass](https://github.com/PredictiveEcology/CBM_vol2biomass.git) is also study-area specific, as @boudewyn2007 parameters are dominant species and [ecozone](https://predictiveecology.github.io/spadesCBM/lexicon.html) specific. 

[CBM_defaults](https://github.com/PredictiveEcology/CBM_defaults.git), and [CBM_vol2biomass](https://github.com/PredictiveEcology/CBM_vol2biomass.git) have one [event](https://predictiveecology.github.io/spadesCBM/lexicon.html) (`init`), and need to be run only once (note that they will be [cached](https://predictiveecology.org/training/_book/Caching.html) in our example).[CBM_dataPrep_SK](https://github.com/PredictiveEcology/CBM_dataPrep_SK.git) processes most of the data in its `init` event, but also has a second event where the annual disturbances are processed (`readDisturbanceEvents`). This second event enables dynamic linkages to disturbance models. 
...
We can use the SpaDES.core generated `moduleDiagram`
![](https://raw.githubusercontent.com/PredictiveEcology/spadesCBM/main/manual/images/spadesCBMmoduleDiagram.png){fig-align="center" width="500"}

Or a version of my hex diagrams

![](https://raw.githubusercontent.com/PredictiveEcology/spadesCBM/main/manual/images/spadesCBM_HexV1.png){fig-align="center" width="500"}


Several core utilities to [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git) are provided by the [`CBMutils`](https://github.com/PredictiveEcology/CBMutils/tree/development) package, available on GitHub. The [SpaDES](https://SpaDES.PredictiveEcology.org) platform has a much broader scope then our use of it in [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git). We provide some basic [SpaDES](https://SpaDES.PredictiveEcology.org) definitions [here](https://predictiveecology.github.io/spadesCBM/lexicon.html) so that users can perform [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git) simulations without having to learn all the [SpaDES](https://SpaDES.PredictiveEcology.org) capacities. [`CBMutils`](https://github.com/PredictiveEcology/CBMutils/tree/development) functions used in our example are listed with a brief explanation in [this](https://predictiveecology.github.io/spadesCBM/CBMutils.html) subsequent section. Active development in [`CBMutils`](https://github.com/PredictiveEcology/CBMutils/tree/development) and all [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git) modules is underway.
...
There are 18 internal carbon pools in [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git), representing a stand or pixel (Table 1), and two pools that take carbon out of the system, Products and the Atmosphere. The carbon transfers are dictated by matrices (available in the defaults information) which specify the proportion of carbon moving from one pool ([source_pool](https://predictiveecology.github.io/spadesCBM/lexicon.html) to a [sink_pools](https://predictiveecology.github.io/spadesCBM/lexicon.html)). Matrices for growth are populated with the user-provided growth information and are the main change agent in the system, providing carbon input to the stand. Default parameters for biomass and dead organic mater (DOM) turnover, decay, and soil mixing are either at the provincial/territorial, ecozone, or Canada-wide scale (see Table 1 in @kurz2009). Note that parameters can easily be modified from their defaults in a [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git) simulation. Disturbances matrices representing carbon-transfers during disturbances like fire or harvesting, are available in the defaults (read-in via [CBM_defaults](https://github.com/PredictiveEcology/CBM_defaults.git)) for all managed forests of Canada. 
